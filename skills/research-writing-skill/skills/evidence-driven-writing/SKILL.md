...
- `refs/citation-verification.csv`
...
| Source ID | Citation | Ledger 引用编号 | Source type | Abstract-level finding | Usable fact | Supported claim | Citation slot | Risk |
|---|---|---|---|---|---|---|---|---|
...
- Every source used in manuscript prose must have a matching row in `refs/citation-verification.csv`.
- The `Citation slot` must match `论文引用位置` in the CSV after the paragraph is drafted.
- Weak, indirect, partial, or metadata-only sources must be marked as needing manual verification; never promote machine verification to manual verification.
- If a source cannot be entered into the CSV with at least a traceable title/author/year/source or URL/DOI/database handle, do not use it as a formal citation.
